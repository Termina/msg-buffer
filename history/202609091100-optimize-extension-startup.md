...
- Replace the full Feather icon dependency with two local inline SVGs, and dynamically import Axios only when the Anthropic provider is used.
...
- 使用两个本地内联 SVG 替代完整 Feather 图标依赖，并仅在调用 Anthropic 服务时动态加载 Axios。
